...
const TextWithMagicSymbol: React.FC<Props> = ({text, cardsInCombo = [], includeCardLinks}: Props) => {
        ...
        <span key={i} className={styles[`${item.nodeType}Container`]}>
          ...
          {item.nodeType === 'card' &&
            <CardTooltip cardName={item.cardName}>
              {includeCardLinks ? <CardLink name={item.cardName || ''}>{item.value}</CardLink> : <span>{item.value}</span>}
            </CardTooltip>
          }
          {item.nodeType !== 'card' && item.nodeType !== 'image' && <span>{item.value}</span>}
        </span>
      ))}
    </span>
  )
