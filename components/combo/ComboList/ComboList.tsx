...
const ComboList = ({title, cardsInCombo = [], includeCardLinks, showNumbers, iterations, id, className}: Props) => {
      ...
      <div className="pr-6 py-4">
        ...
        <ol className={`${styles.comboList} ${showNumbers && 'list-decimal'}`}>
          {iterations.map((item, index) => (
            <li key={`${title}-${index}`}>
              <TextWithMagicSymbol text={item} cardsInCombo={cardsInCombo} includeCardLinks={includeCardLinks} />
            </li>
          ))}
          {iterations.length === 0 && Array.from(Array(numberOfPlaceHolderItems).keys()).map((index) => (
            <li key={index}>
